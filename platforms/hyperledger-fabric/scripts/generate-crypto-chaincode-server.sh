#!/bin/bash
if [ $# -lt 6 ]; then
	echo "Usage : . $0 <namespace> <nodename> <id-name> <id-type> <affiliation> <subject> <server-addr>"
	exit
fi
...
if [ ! -d "${ORG_HOME}/chaincode/${CHAINCODE_NAME}/v${VERSION}" ]; then # if user certificates do not exist
	...
	mkdir ${ORG_HOME}/chaincode/${CHAINCODE_NAME}/v${VERSION}/msp/admincerts
	cp ${ORG_HOME}/chaincode/${CHAINCODE_NAME}/v${VERSION}/msp/signcerts/* ${ORG_HOME}/chaincode/${CHAINCODE_NAME}/v${VERSION}/msp/admincerts/${ORG_USER}-cert.pem

	mkdir -p ${ORG_CYPTO_FOLDER}/chaincode/${ORG_USER}
	cp -R ${ORG_HOME}/chaincode/${CHAINCODE_NAME}/v${VERSION}/msp ${ORG_CYPTO_FOLDER}/chaincode/${ORG_USER}

	# Get TLS cert for user and copy to appropriate location
	fabric-ca-client enroll -d --enrollment.profile tls -u https://${ORG_USER}:${ORG_USERPASS}@${CA} -M ${ORG_HOME}/chaincode/${CHAINCODE_NAME}/v${VERSION}/tls --tls.certfiles ${ROOT_TLS_CERT} --csr.hosts "${HOST}"

	# Copy the TLS key and cert to the appropriate place
	mkdir -p ${ORG_CYPTO_FOLDER}/chaincode/${ORG_USER}/tls
	cp ${ORG_HOME}/chaincode/${CHAINCODE_NAME}/v${VERSION}/tls/keystore/* ${ORG_CYPTO_FOLDER}/chaincode/${ORG_USER}/tls/client.key
	...
	CERT_DATETIME=$(date -d "$(echo $(openssl x509 -noout -enddate < ${ORG_HOME}/chaincode/${CHAINCODE_NAME}/v${VERSION}/msp/signcerts/cert.pem) | sed 's/notAfter=//g')" +'%Y%m%d%H%M%S')
