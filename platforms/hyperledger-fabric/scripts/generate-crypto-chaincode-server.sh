...
if [ ! -d "${ORG_HOME}/client${ID_NAME}" ]; then # if user certificates do not exist
	...
	fabric-ca-client enroll -d --enrollment.profile tls -u https://${ORG_USER}:${ORG_USERPASS}@${CA} -M ${ORG_HOME}/client${ID_NAME}/tls --tls.certfiles ${ROOT_TLS_CERT} --csr.hosts "${HOST}"
